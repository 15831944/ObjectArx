...
#include "AddBlockReference.h"
/////////////////////////////////////////////////////////////////////////////
// CBlockViewDlg dialog

extern CAcExtensionModule BlockViewDLL;

CBlockViewDlg::CBlockViewDlg(CWnd * pParent)
...
BEGIN_MESSAGE_MAP(CBlockViewDlg, CAcUiDialog)
	...
	ON_COMMAND(ID_Z_SCALE_DIV_10, OnZScaleDiv10)
	ON_COMMAND(ID_Z_SCALE_DIV_5, OnZScaleDiv5)
	ON_COMMAND(ID_Z_SCALE_DIV_2, OnZScaleDiv2)
	ON_COMMAND(ID_Z_SCALE_ORIGIN, OnZScaleOrigin)
	ON_COMMAND(ID_Z_SCALE_MUL_2, OnZScaleMul2)
	ON_COMMAND(ID_Z_SCALE_MUL_5, OnZScaleMul5)
	ON_COMMAND(ID_Z_SCALE_MUL_10, OnZScaleMul10)
END_MESSAGE_MAP()
...
void CBlockViewDlg::OnAddentity()
{
	...
	{
		//AcDbCircle *line = new AcDbCircle(AcGePoint3d(0, 0, 0), AcGeVector3d::kZAxis, 100);
		...
		pBT->close();
		//CusEntityByBlkRef *line = new CusEntityByBlkRef(blockId);
		AddBlockReference line;
		line.setBlockRef(blockId);
...
#if ARX==23
		mPreviewCtrl.mpView->setVisualStyle(AcGiVisualStyle::kGouraud);
#else
...
void CBlockViewDlg::OnFileOutput()
{
	...
	if (status == RTNORM)
	{
		...
		//if (!CreateAtilImage(mPreviewCtrl.mpView, sizeRect.Width(), sizeRect.Height(), 32, 0, pFileName, false))
...
void CBlockViewDlg::OnFileAcGsConfig()
{
	...
	if (pGsMgr != NULL)
	{
		...
		AcGsGraphicsKernel *pClassFactory = AcGsManager::acquireGraphicsKernel(descriptor);
		//AcGsGraphicsKernel *pClassFactory = pGsMgr->acquireGraphicsKernel(descriptor);
#else
...
#if ARX ==23
		AcGsManager::releaseGraphicsKernel(pClassFactory);
#endif
	}
}
...
void CBlockViewDlg::OnZScale(const double newScale)
{
	if (newScale == m_zScale)
		return;

	// Update the menu
	CAcModuleResourceOverride res;
	CMenu *menu = GetMenu();
	menu->CheckMenuItem(ID_Z_SCALE_DIV_10, (newScale == 0.1) ? MF_CHECKED : MF_UNCHECKED);
	menu->CheckMenuItem(ID_Z_SCALE_DIV_5, (newScale == 0.2) ? MF_CHECKED : MF_UNCHECKED);
	menu->CheckMenuItem(ID_Z_SCALE_DIV_2, (newScale == 0.5) ? MF_CHECKED : MF_UNCHECKED);
	menu->CheckMenuItem(ID_Z_SCALE_ORIGIN, (newScale == 1.0) ? MF_CHECKED : MF_UNCHECKED);
	menu->CheckMenuItem(ID_Z_SCALE_MUL_2, (newScale == 2.0) ? MF_CHECKED : MF_UNCHECKED);
	menu->CheckMenuItem(ID_Z_SCALE_MUL_5, (newScale == 5.0) ? MF_CHECKED : MF_UNCHECKED);
	menu->CheckMenuItem(ID_Z_SCALE_MUL_10, (newScale == 10.0) ? MF_CHECKED : MF_UNCHECKED);

	// Set new Z scale to the model
	AcGeMatrix3d mat;
	mat(2, 2) = newScale;
	m_zScale = newScale;
	mPreviewCtrl.mpModel->setTransform(mat);

	// Refresh view
	refreshView(mPreviewCtrl.mpView);
}

void CBlockViewDlg::OnZScaleDiv10() { OnZScale(0.1); }
void CBlockViewDlg::OnZScaleDiv5() { OnZScale(0.2); }
void CBlockViewDlg::OnZScaleDiv2() { OnZScale(0.5); }
void CBlockViewDlg::OnZScaleOrigin() { OnZScale(1.0); }
void CBlockViewDlg::OnZScaleMul2() { OnZScale(2.0); }
void CBlockViewDlg::OnZScaleMul5() { OnZScale(5.0); }
void CBlockViewDlg::OnZScaleMul10() { OnZScale(10.0); }
