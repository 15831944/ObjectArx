...
{
#if ARX==23
	AcGsManager::releaseGraphicsKernel(mpFactory);
#endif
	if (mpView)
	{
		mpView->eraseAll();
...
void CGsPreviewCtrl::init(HMODULE hRes, bool bCreateModel)
{
	...
	if (!mpManager)
	{
		...
		//mpFactory = mpManager->acquireGraphicsKernel(descriptor);

		//AcGsDevice *offDevice = mpManager->createAutoCADOffScreenDevice(*mpFactory);
		//AcGsModel *pModel = mpManager->createAutoCADModel(*mpFactory);
#else
