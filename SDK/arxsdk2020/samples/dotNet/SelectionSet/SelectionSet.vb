...
Namespace Selection
    ...
    Public Class AcEdSSGetCommand
       ...
       Public Sub CrossWindow()
        ...
        <CommandMethod("TestVBGroup", "Select_CrossingPolygon", CommandFlags.Modal)>
        Public Sub SelectCrossingPolygon()
            Dim db As Database = HostApplicationServices.WorkingDatabase
            Dim doc As Document = Application.DocumentManager.GetDocument(db)
            Dim ed As Editor = doc.Editor

            Dim pt3dArr(5) As Point3d
            ...
            Dim pt3dCol As New Point3dCollection(pt3dArr)
            Dim selRes As PromptSelectionResult = ed.SelectCrossingPolygon(pt3dCol)
            If Not selRes.Status = PromptStatus.OK Then Return

            Dim sSet As Autodesk.AutoCAD.EditorInput.SelectionSet = selRes.Value
            Dim objIds As ObjectId() = sSet.GetObjectIds()
            ed.WriteMessage(vbLf + "The selection set has {0} object", objIds.Length)

            Dim trans As Transaction = db.TransactionManager.StartTransaction()
            Try
                Dim objId As ObjectId
                For Each objId In objIds
                    Dim entity As Entity = objId.GetObject(OpenMode.ForWrite)
                    If entity.GetType() = GetType(Line) Or entity.GetType() = GetType(Circle) Then
                        entity.ColorIndex = 5
                    End If
                Next objId
                trans.Commit()
